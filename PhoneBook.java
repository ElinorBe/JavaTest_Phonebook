...
public class PhoneBook {
    ...
    public void DeleteContact_ByName(String name) {
        flag = false; /* The boolean variable is to know if there was at least 1 user that removed.
        If so, then the flag gets true, if not the flag stay false and that's how I know that no contact has been removed. */
        if (PhoneBook.size() > 0) {
    ...
    public void SearchContact_ByName(String name) { /**/
        flag = false; // The boolean variable is to know if at least 1 user has found.
        // If so, then the flag gets true, if not the flag stay false and that's how I know that no contact has been found.
        if (PhoneBook.size() > 0) {
            for (Contact c : PhoneBook)
    ...
    public void SortPhonebook_Ascending() {
        if (temp.size() > 0) {
            System.out.println(temp);
            Collections.sort(temp, (c1, c2) -> c1.getContactName().compareTo(c2.getContactName())); /* Function gets the phonebook to be sorted, and c1 & c2 – the comparators to determine the order of the list.
    ...
    /* Function sorting Phonebook by descending order (from large to small), by name, using "Collections.sort".
     * Operations are done on temp variable in order to not change the original phonebook */
    public void SortPhonebook_Descending() {
        if (temp.size() > 0) {
            System.out.println(temp);
            Collections.sort(temp, Collections.reverseOrder((c1, c2) -> c1.getContactName().compareTo(c2.getContactName()))); /* Function gets the phonebook to be sorted, and c1 & c2 – the comparators to determine the order of the list.
            The comparators are being compared each one to the next one. */
            System.out.println(temp);
            temp = PhoneBook;
        } else System.out.println("PhoneBook is empty!!");
    ...
    /* Phonebook is being safe to a txt file */
    public void saveToTxtFile() throws IOException {
        if (PhoneBook.size() > 0) {
            File myPhoneBook = new File("PhoneBook.txt"); /*New file named "PhoneBook.txt" */
            /* In the next try-catch I create new file with the above name, and at the same time I check if the file is already exists.
             * If it exits or not - a relevant message will appear, and if there is some other error, an exception will be thrown. */
            try {
            ...
            /* In the next try-catch the loop go through each line from the phonebook and write it to the file. */
            try {
